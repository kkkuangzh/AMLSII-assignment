...
def prepare_patches(LR_train_bicubic_path, HR_train_path, patch_height, patch_width, patch_num, up_scale):
...
def train_bi_mse_x2_model(LR_patch_train, HR_patch_train, training_epoch):
    print('training bi_mse_x2_model...')
    mse_model_x2 = final_model(48, 48, 3)
    mse_model_x2.compile(optimizer=Adam(lr=1e-4), loss='mse', metrics=['accuracy'])
    history1 = mse_model_x2.fit(LR_patch_train, HR_patch_train, epochs=training_epoch, verbose=1,
                                batch_size=4, validation_split=0.2)
    # mse_model_x2.save('./Track1/mse_bicubic_X2.h5')
    return history1


# train model using perceptual loss as loss function
def train_bi_perceptual_x2_model(LR_patch_train, HR_patch_train, training_epoch):
    print('training bi_perceptual_x2_model...')
    ...
    history2 = perceptual_loss_model_x2.fit(LR_patch_train, HR_patch_train, epochs=training_epoch, verbose=1,
                                            batch_size=4, validation_split=0.2)
    # perceptual_loss_model_x2.save('./Track1/perceptual_bicubic_X2.h5')
    return history2
...
# ---------------------------------- X4 model training --------------------------------------
# train model using MSE as loss function
def train_bi_mse_x4_model(LR_patch_train, HR_patch_train, training_epoch):
    print('training bi_mse_x4_model...')
    mse_model_x4 = build_mse_bi_x4_model()
    # train bicubic_X4 model
    mse_model_x4.compile(optimizer=Adam(lr=1e-4), loss='mse', metrics=['accuracy'])
    history3 = mse_model_x4.fit(LR_patch_train, HR_patch_train, epochs=training_epoch, verbose=1, batch_size=4,
                                validation_split=0.2)
    # mse_model_x4.save('./Track1/mse_bicubic_X4.h5')
    return history3


def build_mse_bi_x4_model():
    # load trained MSE_X2 model
    x2_model = load_model('./Track1/mse_bicubic_X2.h5', custom_objects={'tf': tf, 'perceptual_loss_x2': perceptual_loss_x2})
...
# train model using perceptual loss as loss function
def train_bi_perceptual_x4_model(LR_patch_train, HR_patch_train, training_epoch):
    print('training bi_perceptual_x4_model...')
    perceptual_model_x4 = build_perceptual_bi_x4_model()

    # train bicubic_X4 model
    perceptual_model_x4.compile(optimizer=Adam(lr=1e-4), loss=perceptual_loss_x4, metrics=['accuracy'])
    history4 = perceptual_model_x4.fit(LR_patch_train, HR_patch_train, epochs=training_epoch, verbose=1, batch_size=4,
                                       validation_split=0.2)
    # perceptual_model_x4.save('./Track1/perceptual_bicubic_X4.h5')
    return history4


def build_perceptual_bi_x4_model():
    # load trained perceptual_X2 model
    x2_model = load_model('./Track1/perceptual_bicubic_X2.h5',
