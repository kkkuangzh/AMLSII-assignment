...
def load_unknown_test_images(LR_valid_unknown_path, HR_valid_path):
    print('Loading test images...')
    LR_valid_imgs = load_data_from_dir(LR_valid_unknown_path)
    HR_valid_imgs = load_data_from_dir(HR_valid_path)

    return LR_valid_imgs, HR_valid_imgs


def evaluate_on_DIV2K_unknown(LR_valid_unknown_path, HR_valid_path, model_path, test_num, stride, up_scale):
    LR_valid_imgs, HR_valid_imgs = load_unknown_test_images(LR_valid_unknown_path, HR_valid_path)
    ...
    PSNR_val, SSIM_val = calculate_psnr_ssim_unknown(predicted_HR_list, HR_valid_imgs)
    return PSNR_val, SSIM_val


def calculate_psnr_ssim_unknown(predicted_HR_list, HR_valid_imgs):
    PSNR_val = []
    SSIM_val = []
